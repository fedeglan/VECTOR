...
- **Phase B — the Tier-2 relay-runner is implemented** (`src/orchestration/runner/`, `vector-run`/`vector-revert` entry points). Control flow lifted from the frozen `fsm_sim.py` and executed against the REAL runner via deterministic `claude`/`gh`/`git` shims: the 8 spec scenarios + 22 live-behavior scenarios (incl. a real `SIGKILL` mid-build → state-only resume) + 9 red-team regressions = **39/39**; `fsm_sim.py` untouched, still 8/8. A 16-agent adversarial red-team found **9 confirmed violations, all fixed** (crash-safe atomic merge, per-issue wall budget, final-block-only exit parsing, PR-ownership verification, fail-fast POLICY, full escalation template, unclean-phase refusal). Not yet run with live `claude -p` builders — blocked on operator credentials (headless auth + machine user); runbook in the runner README. See DECISIONS §7.

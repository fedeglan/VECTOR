...
class Hooks:
    ...
    def log_escalation(self, iid, reason, question):
        with open(self.escalations_path, "a") as fh:
            fh.write(f"\n## {now_iso()} — {iid} — {reason} (needs-human)\n"
...
def run_issue(st, policy, hooks, iid):
    ...
    while True:
        ...
        if remaining <= 0:
            ...
            hooks.log_escalation(iid, "budget", "minutes_per_issue exhausted")
            return "escalated"
        it["attempts"] += 1
        ...
        if res.outcome == "blocked":
            ...
            scheduler.escalate(st, policy, iid, "ambiguity", count_failure=False,
                               question=q)
            hooks.log_escalation(iid, "ambiguity", q)
        ...
        if res.outcome == "spec-conflict":
            ...
            scheduler.escalate(st, policy, iid, "spec-conflict", count_failure=False,
                               positions=p)
            hooks.log_escalation(iid, "spec-conflict", p)
        ...
        if res.outcome == "failed":
            if it["attempts"] >= budgets["build_attempts"]:
                ...
                hooks.log_escalation(iid, "budget", res.data.get("reason", ""))
                return "escalated"
        ...
        if not pr:
            ...
            if it["attempts"] >= budgets["build_attempts"]:
                ...
                hooks.log_escalation(iid, "budget", "pr-open without PR number")
                return "escalated"
        ...
        while True:
            ci_state, names, tail = gates.wait_ci(
                ...
                timeout_secs=budgets["minutes_per_issue"] * 60)
            if ci_state == "infra-red":
                ...
                hooks.log_escalation(iid, "infra", f"CI infra failure: {names}")
                return "escalated"
            break
        if ci_state == "red":
            if it["attempts"] >= budgets["build_attempts"]:
                ...
                hooks.log_escalation(iid, "budget", f"CI red: {names}")
                return "escalated"
        ...
        while True:
            ...
            if rres.outcome == "failed":
                ...
                hooks.log_escalation(iid, "infra", "reviewer failed twice")
                return "escalated"
            break
        ...
        if rres.data.get("spec_conflict"):
            ...
            hooks.log_escalation(iid, "spec-conflict", summary)
            return "escalated"
        ...
        # ---- VERDICT -----------------------------------------------------------
        if verdict == "APPROVED":
            tag = gates.merge_pr(pr, iid, cwd, policy["merge"]["target"],
                                 gh_issue=it.get("gh"))
            it["tag"] = tag
            scheduler.merged(st, iid)
            hooks.log_session(_session_line(st, iid))
        ...
        if it["rejections"] > policy["budgets"]["review_cycles"]:  # 3rd rejection
            ...
            hooks.log_escalation(iid, "review-deadlock",

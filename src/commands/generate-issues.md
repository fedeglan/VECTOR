...
- **Every issue has a non-empty `phase`, and that `phase` is a member of the ledger's own `phases`.**
- **Every issue has a non-empty `files`.** An issue that cannot name a single file it will touch is not
  decomposed enough to build.
- The Markdown and the JSON list the **same** issue ids (count and set match).
- Every PRD story is covered by ≥1 issue.

> **Why `phase` and `files` are checked here.** The schema in §5 declares both, and until this check
> existed nothing verified either — so a ledger could omit them on 100% of its issues and still report
> success. Measured across four backlogs generated by this command: two carried both fields on every
> issue, two omitted both on every issue, and **all four reported success**, because the check never
> looked. A field that is declared but never verified is documentation, not a contract — *a gate that
> cannot fail is not a gate*, applied to the generator's own checker. The worse half is that the missing
> check did not merely fail to catch the defect: it removed the signal that the runs had diverged at all.

```bash
python3 - <<'PY'
import json, sys
d = json.load(open('.vector/issues.json'))
iss = d['issues']
ids = {i['id'] for i in iss}
phases = set(d.get('phases', []))
bad = []
...
for i in iss:
    ...
    for x in i.get('deps', []):
        if x not in ids:
            bad.append(f"{tag}: dangling dep {x!r}")
    if not i.get('verification'):
        bad.append(f"{tag}: empty `verification`")
    # Declared in the §5 schema; enforced here. Name the offending ids — a bare
    # `assert all(...)` over 100 issues tells you nothing about which one is wrong.
    if not i.get('phase'):
        bad.append(f"{tag}: missing/empty `phase`")
    elif phases and i['phase'] not in phases:
        bad.append(f"{tag}: phase {i['phase']!r} not in ledger phases {sorted(phases)}")
    if not i.get('files'):
        bad.append(f"{tag}: missing/empty `files`")

g = {i['id']: set(i.get('deps', [])) for i in iss}
done = set()
for _ in range(len(ids) + 1):
    progressed = {k for k, dep in g.items() if k not in done and dep <= done}
    if not progressed:
        break
    done |= progressed
if done != ids:
    bad.append(f"CYCLE in the dependency DAG (unreachable): {sorted(ids - done)}")

if bad:
    print(f"LEDGER SELF-CHECK FAILED ({len(bad)}):")
    for b in bad:
        print("  -", b)
    sys.exit(1)
print(f"{len(ids)} issues, ledger consistent + acyclic; phase+files present on all")
PY
```
